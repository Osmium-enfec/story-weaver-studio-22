...
const Body = z.discriminatedUnion("action", [
  ...
  z.object({
    action: z.literal("assignEpisodeUser"),
    id: z.string().uuid(),
    assigned_user_id: z.string().uuid().nullable(),
  }),
  z.object({
  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        if (data.action === "assignEpisodeUser") {
          if (!asAdmin) return jsonError("Admin only.", 403);
          try {
            const assignee = await resolveAssignee(data.assigned_user_id);
            const { localAssignEpisodeAssignee } = await import(
              "@/lib/local-projects-db"
            );
            const updated = await localAssignEpisodeAssignee(data.id, assignee);
            return jsonResponse(
              normalizeProjectRecord(updated as unknown as Record<string, unknown>),
            );
          } catch (e) {
